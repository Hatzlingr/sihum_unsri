...
>
    <div class="mx-auto flex max-w-7xl items-center justify-between gap-5 px-5 py-4">
        ...
        <div class="hidden items-center gap-3 lg:flex">
            @auth
                <a href="#" class="text-sm font-semibold text-content-sub">
                    {{ auth()->user()->username }}
                </a>
                <a
                    href="{{ match(auth()->user()->role) { 'Admin' => route('admin.dashboard'), 'Pengelola' => route('pengelola.dashboard'), default => route('mahasiswa.dashboard') } }}"
                    class="rounded-full px-4 py-2 text-sm font-bold text-content-sub transition hover:bg-bg-surface hover:text-brand"
                >
                    Dashboard
                </a>
                <form method="POST" action="{{ route('logout') }}" class="inline">
                    @csrf
                    <button type="submit" class="inline-flex items-center gap-2 rounded-full bg-brand px-5 py-2.5 text-sm font-bold text-white shadow-lg shadow-brand/20 transition duration-300 hover:-translate-y-0.5 hover:bg-brand/90">
                        Keluar
                    </button>
                </form>
            @else
    ...
    >
        <div class="mx-5 mb-5 rounded-4xl bg-bg-base p-3 shadow-xl shadow-slate-200/80">
            ...
            <div class="mt-3 grid gap-3 @auth grid-cols-1 @else grid-cols-2 @endauth">
                @auth
                    <a
                        href="{{ match(auth()->user()->role) { 'Admin' => route('admin.dashboard'), 'Pengelola' => route('pengelola.dashboard'), default => route('mahasiswa.dashboard') } }}"
                        class="rounded-2xl bg-bg-surface px-4 py-3 text-center text-sm font-bold text-content-main transition hover:bg-brand-soft hover:text-brand"
                    >
                        Dashboard
                    </a>
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="w-full rounded-2xl bg-brand px-4 py-3 text-center text-sm font-bold text-white shadow-md shadow-brand/20 transition hover:bg-brand/90">
                            Keluar
                        </button>
                    </form>
                @else
