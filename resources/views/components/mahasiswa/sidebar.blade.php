...
<aside
    ...
    <div class="flex h-24 items-center gap-4 border-b border-border-soft px-6">
        ...
        <div class="min-w-0">
            ...
            <p class="truncate text-sm font-semibold leading-tight text-content-sub">
                {{ auth()->user()->name ?? 'Mahasiswa' }}</p>
        </div>
        <button type="button"
            ...
            @click="sidebarOpen = false" aria-label="Tutup menu mahasiswa">
            ...
            <div>
                ...
                <div class="space-y-1">
                    @foreach ($items as $item)
                        ...
                        @endphp
                        <x-mahasiswa.nav-link :href="$routeUrl($item['route'], $item['fallback'])" :icon="$item['icon']"
